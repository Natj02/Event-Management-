...
function GetTasks(){
    getList('groups/TASK_CONTAINER').then((tasks)=>{
        tasks.forEach(tsk => {
            AddTaskAsListItem(tsk);
        });
    })
}

function AddTaskAsListItem(tsk){
    let value = tsk.val();

    console.log(value);

    let table = tasksDiv.querySelector('.contentTable tbody');

    let row = document.createElement('tr');
    let taskTitleCell = document.createElement('td');
    let taskProgressCell = document.createElement('td');

    taskTitleCell.textContent = value.task;
    taskProgressCell.textContent = value.status.toUpperCase();

    row.appendChild(taskTitleCell);
    row.appendChild(taskProgressCell);
